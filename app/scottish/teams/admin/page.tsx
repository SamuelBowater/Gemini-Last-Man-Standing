...
export default function ScottishTeamsAdminPage() {
  ...
  return (
    <div className="max-w-[480px] mx-auto px-4 py-16">
      ...
      <div className="text-center">
        <Link href="/scottish/teams" className="text-text-dim text-[12px] font-mono hover:text-accent">
          ← Back to the pool
        </Link>
      </div>
    </div>
...
function AdminDashboard() {
  ...
  const refresh = useCallback(async () => {
    const [pubState, fixturesRes] = await Promise.all([
      api("/api/scot-team-state"),
      api(`/api/admin/scot-team-fixtures?gw=${selectedGW}`),
    ]);
    ...
    <div className="max-w-[640px] mx-auto px-4 pb-24 pt-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <h1 className="font-display text-3xl mt-1">🏴 Scottish Team Survival admin</h1>
        </div>
        <GhostButton
      ...
      <div className="text-center mt-8">
        <Link href="/scottish/teams" className="text-text-dim text-[11px] font-mono hover:text-accent">
          ← Back to the pool
        </Link>
      </div>
    </div>
  ...
  return (
    <Panel>
      ...
      <Sub>
        ...
        shared with the Scottish Player Picks trial, since fixtures live in the same table. Only
        some gameweeks may be published this far ahead, so re-run this after each round finishes.
      </Sub>
...
function ManualFixturesPanel({
  ...
  async function add() {
    ...
    setError("");
    try {
      await api("/api/admin/scot-team-fixtures", {
        method: "POST",
        body: JSON.stringify({
          home,
          away,
  ...
  async function remove(id: number) {
    await api(`/api/admin/scot-team-fixtures?id=${id}`, { method: "DELETE" });
    onChange();
  }
  ...
  return (
    <Panel>
      ...
      <div className="flex gap-2.5 mb-2.5">
        <TextInput list="scot-teams-admin" value={home} onChange={(e) => setHome(e.target.value)} placeholder="Home team" />
        <TextInput list="scot-teams-admin" value={away} onChange={(e) => setAway(e.target.value)} placeholder="Away team" />
      </div>
      <datalist id="scot-teams-admin">
...
function TeamResultsPanel({
  ...
  const fetchFixtures = useCallback(async () => {
    const res = await api(`/api/scot-team-fixtures?gw=${gameState.currentGW}`);
  ...
  async function undo() {
    ...
    setMsg("");
    try {
      const res = await api("/api/admin/undo-results", { method: "POST", body: JSON.stringify({ game: "scot-teams" }) });
  ...
  async function apply() {
    ...
    setMsg("");
    try {
      const res = await api("/api/admin/scot-team-results", { method: "POST", body: JSON.stringify({ winningTeams }) });
