...
function downloadCSV(report: TeamGameweekReport) {
  ...
  a.href = url;
  a.download = `scottish-team-survival-gameweek-${report.gw}-selections.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

export default function ScottishTeamStandingsPage() {
  ...
  const load = useCallback((targetGw?: number) => {
    return api(`/api/scot-team-gameweek${targetGw ? `?gw=${targetGw}` : ""}`).then((data: TeamGameweekReport) => {
      setReport(data);
      setGw(data.gw);
    });
  }, []);
