...
export const POST = withErrors(async (req: NextRequest) => {
  ...
  const { rows: gsRows } = await pool.query("SELECT current_gw, phase, season FROM scot_game_state WHERE id = 1");
  const gs = gsRows[0];
  if (gs.phase !== "picking") {
  ...
  const { rows: meRows } = await pool.query("SELECT scot_status FROM participants WHERE id = $1", [participantId]);
  if (!meRows[0] || meRows[0].scot_status === "eliminated") {
  ...
  await pool.query(
    `INSERT INTO scot_picks (gw, participant_id, forward, midfielder, defender)
     VALUES ($1, $2, $3, $4, $5)
...
export const DELETE = withErrors(async () => {
  ...
  await pool.query("DELETE FROM scot_picks WHERE gw = $1 AND participant_id = $2", [gs.current_gw, participantId]);
