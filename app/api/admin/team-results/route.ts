...
export const POST = withErrors(async (req: NextRequest) => {
  ...
  try {
    ...
    const { rows: gsRows } = await client.query(
      "SELECT current_gw, season, phase FROM team_state WHERE id = 1 FOR UPDATE"
    );
    const gs = gsRows[0];
    if (gs.phase !== "picking") {
    ...
    const { rows: picks } = await client.query(
      `SELECT tp.participant_id, tp.team
       ...
       WHERE tp.gw = $1 AND p.can_play_teams = true`,
      [gs.current_gw]
    );
    ...
    const { rows: alivePlayers } = await client.query(
      "SELECT id FROM participants WHERE team_status != 'eliminated' AND can_play_teams = true"
    );
    ...
    const { rows: stillAlive } = await client.query(
      "SELECT count(*)::int AS n FROM participants WHERE team_status != 'eliminated' AND can_play_teams = true"
    );
