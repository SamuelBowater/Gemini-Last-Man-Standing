...
export const POST = withErrors(async (req: NextRequest) => {
  ...
  try {
    ...
    const { rows: gsRows } = await client.query(
      "SELECT current_gw, phase FROM scot_game_state WHERE id = 1 FOR UPDATE"
    );
    const gs = gsRows[0];
    if (gs.phase !== "picking") {
    ...
    const { rows: picks } = await client.query(
      "SELECT participant_id, forward, midfielder, defender FROM scot_picks WHERE gw = $1",
      [gs.current_gw]
    );
    const { rows: alivePlayers } = await client.query(
      "SELECT id FROM participants WHERE can_play_scot_players = true AND scot_status != 'eliminated'"
    );
    ...
    if (eliminatedIds.length > 0) {
      await client.query(
        `UPDATE participants SET scot_status = 'eliminated', scot_eliminated_gw = $1 WHERE id = ANY($2::int[])`,
        [gs.current_gw, eliminatedIds]
      );
    }

    await client.query(
      `INSERT INTO scot_results (gw, scorers) VALUES ($1, $2)
    ...
    const { rows: stillAlive } = await client.query(
      "SELECT count(*)::int AS n FROM participants WHERE can_play_scot_players = true AND scot_status != 'eliminated'"
    );
    ...
    await client.query("UPDATE scot_game_state SET phase = $1, current_gw = $2 WHERE id = 1", [
      newPhase,
      newGW,
    ]);
