...
export const GET = withErrors(async (req: NextRequest) => {
  ...
  const { rows: gsRows } = await pool.query(
    "SELECT current_gw, season FROM scot_game_state WHERE id = 1"
  );

  const gs = gsRows[0];

  const gw =
    Number(req.nextUrl.searchParams.get("gw")) || gs.current_gw;

  const { rows } = await pool.query(
    `SELECT id, home, away, kickoff, venue, status, source
     FROM fixtures
     WHERE season = $1
       AND gw = $2
...
export const POST = withErrors(async (req: NextRequest) => {
  ...
  const { home, away, kickoff, gw } = await req.json().catch(() => ({}));
